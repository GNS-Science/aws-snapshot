...
> cycle on **Arkivalist** first. Arkivalist (account `816711409078`) is a separate AWS
> account from the spike/backup account (`595842668254`), has similar backup targets
...
> for NSHM production (`461564345538`). This makes it the ideal first real-world target:
> it validates both the restore lifecycle and cross-account IAM before we touch NSHM data.

- [ ] Confirm Arkivalist S3 bucket names and DynamoDB table ARNs (account `816711409078`)
...
- [ ] Apply same cross-account pattern to NSHM production (`461564345538`) before cutover
